...
class PyInstArchive:
    ...
    def __init__(self, path, try_pyz=False, python_exe=None):
        self.filePath = path
        self.pycMagic = b'\0' * 4
        self.barePycList = [] # List of pyc's whose headers have to be fixed
        self.tryPyzExtraction = try_pyz
    ...
    def _extractPyz(self, name):
        ...
        with open(name, 'rb') as f:
            ...
            pymaj = getattr(sys.version_info, 'major', sys.version_info[0])
            pymin = getattr(sys.version_info, 'minor', sys.version_info[1])
    ...
    def _loadPyzTocWithExternalInterpreter(self, pyzName, tocPosition):
        helperCode = (
            "import base64, json, marshal, os, sys\n"
            "pyz_path = sys.argv[1]\n"
            "toc_position = int(sys.argv[2])\n"
            ...
            "        out.append(['bytes', base64.b64encode(key).decode('ascii'), ispkg, pos, length])\n"
            "    else:\n"
            "        out.append(['str', key, ispkg, pos, length])\n"
        ...
        toc = {}
        for item in rawToc:
            keyType, keyData, ispkg, pos, length = item
            if keyType == 'bytes':
                key = base64.b64decode(keyData)
            else:
                key = keyData
            toc[key] = (ispkg, pos, length)
...
def main():
    ...
    arch = PyInstArchive(args.filename, try_pyz=args.try_pyz, python_exe=args.python_exe)
    if arch.open():
        if arch.checkFile():
